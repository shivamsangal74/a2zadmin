...
const RofferFilter = () => {
  ...
  const { isLoading, error, data, refetch } = useQuery({
    ...
    queryFn: async () => {
      try {
        ...
        const _filter = await getAllFilterByType("roffer");
  ...
  return (
    <DefaultLayout isList={true}>
      ...
      <Popup
        title={isEdit ? "Edit Roffer Filter" : "Add Roffer Filter"}
        isOpen={isOpen}
        onClose={closePopup}
      >
        <div className="">
          ...
          <div className="flex justify-between mt-10">
            {isEdit ? (
              <ButtonLabel
                ...
                label="Edit Roffer Filter"
              />
            ) : (
              <ButtonLabel
                onClick={handleAddCategory}
                label="Add Rofffer Filter"
              />
            )}
            <ButtonLabel
